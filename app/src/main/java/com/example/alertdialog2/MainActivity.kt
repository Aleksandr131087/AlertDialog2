...
import com.example.alertdialog2.R
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        listviewLV.onItemClickListener =
            AdapterView.OnItemClickListener{parent, v, position, id ->
                val userDelete = AlertDialog.Builder(this)
                userDelete.setTitle("Внимание!!!")
                userDelete.setMessage("Вы уверены, что хотите удалить пользователя ${users[position].name}?")
                userDelete.setPositiveButton("Да") { _, _ ->
                    users.removeAt(position)
                    adapter.notifyDataSetChanged()                }
                userDelete.setNegativeButton("Нет", null)
                userDelete.show()
            }
    }
